...
function renderSummary() {
  ...
  $("#checkout-summary").innerHTML = `
    ...
    <p class="muted text-sm mt-4">Free shipping above ${formatMoney(brandConfig.freeShippingAbove)}. COD is highlighted because most Clad Whale customers prefer pay-on-delivery.</p>
  `;
...
function rememberOrder(id) {
  const orders = JSON.parse(localStorage.getItem("cladwhale_orders") || "[]");
  localStorage.setItem("cladwhale_orders", JSON.stringify([id, ...orders].slice(0, 8)));
}
