...
export function renderShell(active = "home") {
  ...
  if (header) {
    header.innerHTML = `
      <nav class="nav-shell">
        <a href="${prefix}index.html" class="brand-lockup" aria-label="Clad Whale home">
          <img src="${prefix}assets/images/logo.png" alt="Clad Whale" class="brand-mark">
          <span>Clad Whale</span>
        </a>
  ...
  if (footer) {
    footer.innerHTML = `
      <div class="footer-grid">
        <div>
          <div class="brand-lockup mb-4">
            <img src="${prefix}assets/images/logo.png" alt="" class="brand-mark">
            <span>Clad Whale</span>
          </div>
          <p class="muted max-w-md">Premium streetwear for everyone. Bold monochrome mood, friendly prices, COD-first shopping.</p>
        </div>
        <div>
          <h3>Shop</h3>
          <a href="${prefix}products/index.html?category=oversized-tees">Oversized Tees</a>
          <a href="${prefix}products/index.html?category=hoodies">Hoodies</a>
          <a href="${prefix}products/index.html?category=joggers">Joggers</a>
          <a href="${prefix}products/index.html?category=footwear">Footwear</a>
        </div>
        <div>
          <h3>Support</h3>
      ...
      <div class="footer-bottom">
        <span>© ${new Date().getFullYear()} Clad Whale. All rights reserved.</span>
  ...
  if (whatsapp) {
    whatsapp.href = `https://wa.me/${brandConfig.supportPhone}?text=${encodeURIComponent("Hi Clad Whale, I need help with shopping.")}`;
  }

  $("#nav-toggle")?.addEventListener("click", () => $("#nav-links")?.classList.toggle("open"));
  $("#newsletter-form")?.addEventListener("submit", (event) => {
    event.preventDefault();
    toast("You are on the Clad Whale list.");
    event.target.reset();
  });
  ...
  return `
    <article class="product-card reveal">
      <a href="${productUrl(product)}" class="product-media">
        <img src="${product.images?.[0] || "../assets/images/logo.png"}" alt="${product.name}" loading="lazy">
        <span>${badge}</span>
      </a>
